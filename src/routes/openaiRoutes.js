...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const systemMessage = extractSystemMessage(req.body?.messages || [])
    const isXcodeRequest = !!(systemMessage && systemMessage.includes('You are currently in Xcode'))
    ...
    const shouldTransformStream = isStream && !isCodexCLI
    ...
    if (!isCodexCLI) {
      ...
      // Codex Responses compatibility: map chat/completions fields
      if (req.body && req.body.input === undefined && Array.isArray(req.body.messages)) {
        const normalizeContentItem = (item, role) => {
          const isAssistant = role === 'assistant'
          if (typeof item === 'string') {
            return { type: isAssistant ? 'output_text' : 'input_text', text: item }
          }
          if (!item || typeof item !== 'object') {
            return null
          }
          if (item.type === 'text' && typeof item.text === 'string') {
            return { type: isAssistant ? 'output_text' : 'input_text', text: item.text }
          }
          if (item.type === 'image_url' && item.image_url && item.image_url.url) {
            return { type: 'input_image', image_url: item.image_url.url }
          }
          if (item.type === 'input_text' && isAssistant) {
            return { type: 'output_text', text: item.text || '' }
          }
          if (item.type === 'output_text' && !isAssistant) {
        ...
        req.body.input = req.body.messages.map((msg) => {
          const role = msg && msg.role ? msg.role : 'user'
          const content = msg ? msg.content : ''
          let contentItems = []

          if (typeof content === 'string') {
            contentItems = [{ type: role === 'assistant' ? 'output_text' : 'input_text', text: content }]
          } else if (Array.isArray(content)) {
            contentItems = content
              .map((item) => normalizeContentItem(item, role))
          ...
          if (contentItems.length === 0) {
            contentItems = [{ type: role === 'assistant' ? 'output_text' : 'input_text', text: '' }]
          }
      ...
      if (
        req.body &&
        req.body.max_output_tokens === undefined &&
    ...
    if (req.body && Object.prototype.hasOwnProperty.call(req.body, 'stream_options')) {
      delete req.body.stream_options
      logger.info('🧹 Removed unsupported stream_options for Codex API')
    }
    ...
    const unhandledErrorStatus =
      upstream.status >= 400 && ![401, 402, 429].includes(upstream.status)

    if (unhandledErrorStatus) {
      let errorData = null
      try {
        if (isStream && upstream.data && typeof upstream.data.on === 'function') {
          ...
          await new Promise((resolve, reject) => {
            ...
            const onData = (chunk) => {
              ...
              if (total < 8192) {
                const remaining = 8192 - total
                chunks.push(buf.slice(0, remaining))
                total += buf.slice(0, remaining).length
              }
            }
            upstream.data.on('data', onData)
            upstream.data.on('end', done)
          ...
          try {
            ...
            errorData = fullResponse
          }
        } else {
          errorData = upstream.data
        }
      } catch (parseError) {
        logger.error('⚠️ Failed to parse OpenAI error response:', parseError)
      }
      ...
      const errorPreview =
        typeof errorData === 'string'
          ...
          : Buffer.isBuffer(errorData)
            ? errorData.toString('utf8', 0, 1000)
            : JSON.stringify(errorData).slice(0, 1000)

      logger.warn(`⚠️ OpenAI upstream error ${upstream.status} (Codex API)`, {
        requestId: req.id,
      ...
      const payload =
        errorData && typeof errorData === 'object' && !Buffer.isBuffer(errorData)
          ? errorData
          : {
              error: {
                message: errorPreview || `Upstream error ${upstream.status}`,
                type: 'upstream_error',
                code: 'upstream_error'
              }
            }
    ...
    // Non-Codex CLI: transform Responses SSE to chat.completion.chunk
    let streamResponseId = null
    let streamCreatedAt = null
    let streamModel = requestedModel || 'gpt-4'
    let roleSent = false
    let finishSent = false
    let doneSent = false

    const ensureStreamId = () => {
      if (!streamResponseId) {
        streamResponseId = `chatcmpl_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`
      }
    }

    const buildChatChunk = (delta, finishReason = null) => {
      ensureStreamId()
      const created = streamCreatedAt || Math.floor(Date.now() / 1000)
      const model = actualModel || streamModel || requestedModel || 'gpt-4'
      return {
        id: streamResponseId,
    ...
    const sendChatChunk = (delta, finishReason = null) => {
      if (res.destroyed) {
        return
      }
      const payload = buildChatChunk(delta, finishReason)
      res.write(`data: ${JSON.stringify(payload)}\n\n`)
    }

    const sendDone = () => {
      if (doneSent || res.destroyed) {
        return
      }
      doneSent = true
      res.write('data: [DONE]\n\n')
    ...
    const extractTextDelta = (eventData) => {
      ...
      if (eventData.delta && typeof eventData.delta.text === 'string') {
        return eventData.delta.text
      }
      if (eventData.part && typeof eventData.part.text === 'string') {
        return eventData.part.text
      }
    ...
    const handleTransformedEvent = (eventData) => {
      ...
      if (eventData.type === 'response.created' && eventData.response) {
        if (eventData.response.id) {
          streamResponseId = eventData.response.id
        }
        if (eventData.response.created_at) {
          streamCreatedAt = eventData.response.created_at
        }
        if (eventData.response.model) {
          streamModel = eventData.response.model
        }
        if (!roleSent) {
          sendChatChunk({ role: 'assistant' })
          roleSent = true
        }
        return
      }

      const textDelta = extractTextDelta(eventData)
      if (textDelta !== null && textDelta !== undefined) {
        if (!roleSent) {
          sendChatChunk({ role: 'assistant' })
          roleSent = true
        }
        if (textDelta !== '') {
          sendChatChunk({ content: textDelta })
        }
        return
      }
    ...
    upstream.data.on('data', (chunk) => {
      try {
        ...
        if (!shouldTransformStream && !res.destroyed) {
          res.write(chunk)
        }
        ...
        for (const event of events) {
          if (event.type === 'data' && event.data) {
            ...
            handleTransformedEvent(event.data)
